...
class RelayControllerTest final : public QObject {
 ...
 private slots:
  ...
  void preferencesSurviveRestartAndValidateFontSize() {
    ...
    first.start();
    relay::Preferences preferences{false, 18};
    preferences.themeId = QStringLiteral("catppuccin-mocha");
    preferences.customTheme = {{QStringLiteral("accent"), QStringLiteral("#ff99cc")}};
    ...
    QCOMPARE(second.state().preferences.themeId, preferences.themeId);
    QCOMPARE(second.state().preferences.customTheme, preferences.customTheme);
    auto invalid = preferences;
    invalid.customTheme.insert(QStringLiteral("accent"), QStringLiteral("invalid"));
    QSignalSpy failure(&second, &relay::RelayController::operationFailed);
    second.setPreferences(invalid);
